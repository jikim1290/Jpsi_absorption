...
void compareVar2(){
 ...
 for(int i=0;i<nfiles;i++){
	fin = new TFile(Form("data/NMF_absonly_out_25option_%.01fmb.root",sigmb[i]),"read");
	...
	leg->AddEntry( gNMF_cent[i], Form("#sigma_{abs} = %.01f fmb",sigmb[i]), "p");
 }
 ...
 TGraphErrors* g_tp1_npart = (TGraphErrors*)fSHINCHON->Get("g_RAA_multdep_fdcor_2S_trento_p1");
 ...
 leg1->SetHeader("Au#font[122]{-}Au, 200 GeV, #varUpsilon(2S)");
 leg1->AddEntry( gSTAR_1S_stat, "STAR", "p");
 leg1->AddEntry( g_tp1_npart, "default SHINCHON", "l");
 leg1->AddEntry( (TObject*)0, "", "");
 
 for(int f=0;f<nfiles;f++){
	leg1->AddEntry( g_tp1_npart_addabs[f], Form("#sigma_{abs} = %.1lf mb",sigmb[f]), "l");
 }
 leg1->Draw();

 c->SaveAs("figs/NMF_Fixwabs_2S.pdf");
}
