...
void compareVarUps2(){
 ...
 for(int i=0;i<nfiles;i++){
	if( i!=2 ) fin = new TFile(Form("data/NMF_absonly_out_r2_%doption.root",5+i*4),"read");
	if( i==2 ) fin = new TFile(Form("data/NMF_absonly_out_r2_%doption.root",9),"read");

	gNMF_cent[i] = new TGraph();
	ffit[i] = new TF1("f1","pol3",0,400);
	ffit[i]->SetLineColor(i+1);
	for(int j=0;j<ncent;j++){
		if( i!=2 ) gAdd = (TGraph*)fin->Get(Form("abs_%dto%d_%doption",centmin[j],centmax[j],5+i*4));
		if( i==2 ) gAdd = (TGraph*)fin->Get(Form("abs_%dto%d_%doption",centmin[j],centmax[j],9));
...
//	g_tp1_npart[f] = (TGraphErrors*)fSHINCHON1->Get(Form("g_RAA_multdep_fdcor_%dS_trento_p1_tform600",f+1));
 ...
 TLegend* leg1 = new TLegend(0.2, 0.645, 0.9, 0.968);
 leg1->SetFillStyle(0);
 leg1->SetTextFont(43);
 leg1->SetTextSize(22);
 leg1->SetLineWidth(0);
 ...
 leg1->AddEntry( (TObject*)0, "default", "");
 leg1->AddEntry( (TObject*)0, "default + absorption", "");
 for(int f=0;f<nfiles;f++){
	if( f!=2 ){
		leg1->AddEntry( g_tp1_npart[f], Form("#varUpsilon(%dS)",f+1), "l");
		leg1->AddEntry( g_tp1_npart_addabs[f], Form("#varUpsilon(%dS)",f+1), "l");
	} else if( f==2 ){
        leg1->AddEntry( g_tp1_npart[f], Form("#varUpsilon(%dS), #tau_{form} = 1.2 fm/#it{c}",2), "l");
        leg1->AddEntry( g_tp1_npart_addabs[f], Form("#varUpsilon(%dS), #tau_{form} = 1.2 fm/#it{c}",2), "l");
	}
 }
 leg1->Draw();

 c->SaveAs("figs/NMF_wabs_Ups2.pdf");
}
