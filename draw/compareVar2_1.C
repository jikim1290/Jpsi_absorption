...
void compareVar2_1(){
 ...
 leg->AddEntry( (TObject*)0, "Au#font[122]{-}Au, 200 GeV, #varUpsilon(2S)", "");
 ...
 for(int i=0;i<nfiles;i++){
	fin = new TFile(Form("data/NMF_absonly_out_25option_%dmb.root",sigmb[i]),"read");
	gNMF_cent[i] = new TGraph();
	ffit[i] = new TF1("f1","pol3",0,400);
	ffit[i]->SetLineColor(i+1);
	for(int j=0;j<ncent;j++){
		gAdd = (TGraph*)fin->Get(Form("abs_%dto%d_25option",centmin[j],centmax[j]));
	...
	leg->AddEntry( gNMF_cent[i], Form("#sigma_{abs} = %d fmb",sigmb[i]), "p");
 }
 ...
 c->SaveAs("figs/NMF_Fixabs_2S_only.pdf");
 ...
 gSTAR_2S_syst->SetMaximum(1.3);
 gSTAR_2S_syst->SetMinimum(0);
 gSTAR_2S_syst->SetTitle(";#it{N}_{part};Nuclear modification factor");

 gSTAR_2S_syst->Draw("A2");
 ...
 TFile* fSHINCHON = new TFile("/Users/junleekim/work/SHINCHON/Final/data/FDCOR_out_AuAu_trento_p1_tform600.root","read");
 TGraphErrors* g_tp1_npart = (TGraphErrors*)fSHINCHON->Get("g_RAA_multdep_fdcor_2S_trento_p1_tform600");
 ...
 TLegend* leg1 = new TLegend(0.184, 0.583, 0.934, 0.957);
 leg1->SetFillStyle(0);
 leg1->SetTextFont(43);
 leg1->SetTextSize(28);
 leg1->SetLineWidth(0);
 ...
 leg1->SetHeader("SHINCHON, Au#font[122]{-}Au, 200 GeV, #varUpsilon(2S)");
 leg1->AddEntry( gSTAR_2S_stat, "STAR", "p");
 ...
 for(int f=0;f<nfiles;f++){
	leg1->AddEntry( g_tp1_npart_addabs[f], Form("#sigma_{abs} = %d mb",sigmb[f]), "l");
 }
 leg1->Draw();

 c->SaveAs("figs/NMF_Fixwabs_2S_1.pdf");
}
